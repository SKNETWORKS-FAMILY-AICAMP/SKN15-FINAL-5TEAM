...
export default function ChatInterface({
  ...
  const { fontSize, chatWindowSize } = useApp();
  ...
  const handleBackgroundChange = (currentImage: string | null | undefined) => {
  ...
  const addMessageWithTypingEffect = async (message: Message): Promise<void> => {
    return new Promise((resolve) => {
      ...
      if (message.imageIndex) {
        console.log(`🖼️ [Frontend] Background update requested: ${message.imageIndex} for message: ${message.text.substring(0, 30)}...`);
        handleBackgroundChange(message.imageIndex);
      }
      ...
      try {
        ...
        for await (const chunk of sendChatMessage(
          ...
          if (chunk.type === 'metadata') {
            ...
            finalResponse = { ...finalResponse, ...metadata };
            ...
            if (metadata.affinity_scores) {
              setAffinityScores(metadata.affinity_scores);
            }
            if (typeof metadata.is_ended === 'boolean') {
              setIsEnded(metadata.is_ended);
            }
            if (metadata.current_image) {
              handleBackgroundChange(metadata.current_image);
            }
          } else if (chunk.type === 'dialogue') {
            if (chunk.dialogue) {
              tempDialogues.push(chunk.dialogue);
              if (chunk.dialogue.image_index) {
                handleBackgroundChange(chunk.dialogue.image_index);
              }
            }
        ...
        // 추가 대화가 있으면 자동 요청
        if (finalResponse.has_more) {
          handleAutoRequest(finalResponse.session_id);
        }
  ...
  */
  ...
  const handleAutoRequest = async (currentSessionId: string, retryCount: number = 0) => {
    ...
    try {
      ...
      if (finalResponse.has_more && !shouldCancelAutoRequest.current) {
        autoRequestTimerRef.current = window.setTimeout(() => {
          handleAutoRequest(finalResponse.session_id, 0);
        }, 1000);
      } else {
        setIsAutoRequesting(false);
      }
  ...
  const sendMessage = async (text: string) => {
    ...
    if (sessionId && characterId) {
      addConversation({
        ...
        title: text, // The first message will be used as the title
        lastMessage: text,
      });
    }
    ...
    try {
      ...
      for await (const chunk of sendChatMessage(backendScenarioId, text, sessionId, '츠구코')) {
        if (chunk.type === 'metadata') {
          const metadata = { ...chunk } as Record<string, any>;
          delete metadata.type;
          finalResponse = { ...finalResponse, ...metadata };

          if (typeof metadata.session_id === 'string' && metadata.session_id !== sessionId) {
            setSessionId(metadata.session_id);
          }
          if (metadata.current_stage) {
            setCurrentStage(metadata.current_stage);
  ...
  const chatWidthClass = {
    ...
    cozy: 'max-w-5xl mx-auto',
    spacious: 'max-w-6xl mx-auto'
  } as const;

  return (
    <div
      className={`${chatWidthClass[chatWindowSize]} w-full h-full flex flex-col md:flex-row bg-gray-100 dark:bg-gray-900`}
      ...
      <div className="w-full md:w-1/2 h-[60vh] md:h-full flex flex-col bg-theme-surface-strong border-l border-theme-card">
        ...
        {isLoading && messages.length === 0 && (
          <div className="flex justify-center items-center py-8">
            ...
            <span className="ml-3 text-gray-500 dark:text-gray-400">시나리오를 불러오는 중...</span>
          </div>
        )}
                ...
                <div className="max-w-lg">
                  <div className={`rounded-xl px-4 py-3 shadow-lg border-2 ${ 
                    isWarning
                      ...
                      : 'bg-gradient-to-r from-pink-50 via-purple-50 to-blue-50 border-purple-200 dark:from-gray-700 dark:via-gray-800 dark:to-gray-700 dark:border-purple-900'
                  }`}> 
                    <p className={`text-center text-sm leading-relaxed font-medium ${ 
                      isWarning ? 'text-red-700' : 'text-gray-700 italic dark:text-gray-300'
                    }`}> 
                      {isWarning && <span className="font-bold">⚠️ 꺾쇄까마귀: </span>}
                      {message.text}
                    </p>
                    <div className={`text-xs text-center mt-1 ${ 
                      isWarning ? 'text-red-400' : 'text-gray-400 dark:text-gray-500'
                    }`}> 
          ...
          return (
            <div
              ...
              <div className={`flex flex-col transition-all duration-500 ${ 
                ...
                {/* 화자 표시 */}
                <div className={`text-xs mb-1 ${message.isUser ? 'text-right text-gray-500 dark:text-gray-400' : 'text-left text-gray-500 dark:text-gray-400'}`}> 
                ...
                <div className="flex items-start">
                  <div
                    className={`px-4 py-3 rounded-2xl flex-1 transition-all duration-500 ${ 
                      message.isUser
                        ...
                        : `text-gray-800 border border-gray-200 rounded-bl-md dark:border-gray-700 dark:text-gray-200 ${isLatestMessage ? '' : 'bg-white dark:bg-gray-800 shadow-sm'}`
                    }`}
                  ...
                  >
                    ...
                    <div className={`text-xs mt-1 ${message.isUser ? 'text-purple-100' : 'text-gray-400 dark:text-gray-500'}`}> 
                      {message.timestamp.toLocaleTimeString('ko-KR', {
                        hour: '2-digit',
                        minute: '2-digit'
                      })}
                  ...
                  {!message.isUser && (
                    <button
                      ...
                      className="ml-2 p-1 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-gray-600 dark:hover:bg-gray-700 dark:text-gray-500 dark:hover:text-gray-300"
                      aria-label="음성으로 듣기"
                    >
        ...
        {isTyping && (
          <div className="flex justify-start mb-4">
            ...
            <div className="flex flex-col max-w-xs lg:max-w-md">
              <div className="text-xs mb-1 text-left text-gray-500 dark:text-gray-400">
                입력 중...
              </div>
              <div className="px-4 py-3 rounded-2xl bg-white border border-gray-200 rounded-bl-md shadow-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        {/* 메시지 입력 영역 */}
        <div className="p-4 border-t border-gray-200 bg-white shrink-0 dark:bg-gray-900 dark:border-gray-700">
          <div className="flex items-center space-x-3">
            <button
              onClick={handleAddClick}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-500 dark:hover:bg-gray-800 dark:text-gray-400"
            >
            ...
            <button
              ...
              className="p-2 hover:bg-purple-100 rounded-full transition-colors text-purple-500 hover:text-purple-600 dark:hover:bg-purple-900 dark:text-purple-400 dark:hover:text-purple-300"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isTyping && !isAutoRequesting && (
              <button
                onClick={handleSkip}
                className="p-2 hover:bg-blue-100 rounded-full transition-colors text-blue-500 hover:text-blue-600 animate-pulse dark:hover:bg-blue-900 dark:text-blue-400 dark:hover:text-blue-300"
                title="대화 건너뛰기 (Skip)"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {(isLoading || isTyping || isAutoRequesting) && (
              <button
                ...
                className="p-2 hover:bg-red-100 rounded-full transition-colors text-red-500 hover:text-red-600 dark:hover:bg-red-900 dark:text-red-400 dark:hover:text-red-300"
                title="입력 활성화 (긴급 리셋)"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-gray-700 placeholder-gray-400 outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all disabled:opacity-50 disabled:cursor-not-allowed resize-none pr-12 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-500"
              />
              <button
                onClick={() => {
                ...
                className="absolute right-2 top-1/2 transform -translate-y-1/2 p-2 text-purple-500 hover:text-purple-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed dark:text-purple-400 dark:hover:text-purple-300"
