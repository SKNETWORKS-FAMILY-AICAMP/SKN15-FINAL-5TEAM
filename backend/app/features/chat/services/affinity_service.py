...
logger = get_parent_logger("AffinityService")
...
# 친밀도 프롬프트
AFFINITY_SYSTEM_PROMPT = """당신은 캐릭터 간 친밀도를 판정하는 전문가입니다.

사용자의 입력과 대화 내용을 분석하여 다음 규칙에 따라 친밀도 변화를 계산하세요:
...
JSON 형식으로 응답하세요:
{{
  "affinity_changes": {{"character_id": change_amount, ...}},
  "reasoning": "판정 이유"
}}"""

AFFINITY_USER_PROMPT_TEMPLATE = """
**시나리오 컨텍스트:**
{scenario_context}
...
**참여 캐릭터:** {participating_characters}

위 정보를 바탕으로 각 캐릭터의 친밀도 변화를 판정하세요.
"""


class AffinityService:
    """
    친밀도 관리 서비스 (Layer 3 - Service)
    ...
    Example:
        service = AffinityService(llm_client=llm)

        affinity_scores = service.update_affinity(
            state=state,
            user_input="탄지로를 응원합니다!",
            dialogues=[{"speaker": "tanjiro", "text": "고마워!"}]
        )
    """
    ...
    def __init__(
        ...
        Args:
            ...
            enable_llm: LLM 사용 여부 (False면 규칙 기반만 사용)
        """
    ...
    async def update_affinity(
        ...
        # LLM을 사용하여 친밀도 변화 계산
        if self.enable_llm:
            affinity_changes = await self._classify_interaction_with_llm(
            ...
            if classification == "on_topic":
                affinity_changes = {
                    char: AFFINITY_RULES["general_interaction"]
        ...
        for character, change in affinity_changes.items():
            old_score = current_affinity.get(character, 0)
            new_score = max(0, old_score + change)  # 최소값 0
            current_affinity[character] = new_score

            logger.info("update_affinity", f"💜 {character}: {old_score} → {new_score}",
                       change=change)
...
""".strip()
    ...
    async def _classify_interaction_with_llm(
        ...
        # 사용자 프롬프트 구성
        user_prompt = AFFINITY_USER_PROMPT_TEMPLATE.format(
            scenario_context=scenario_context,
            current_stage=current_stage,
            user_input=user_input,
        ...
        try:
            ...
            response_text = await self.llm_client.call(
                system_prompt=AFFINITY_SYSTEM_PROMPT,
            ...
            logger.error("_classify_interaction_with_llm", f"LLM classification failed: {exc}")
