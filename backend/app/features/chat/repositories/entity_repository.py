...
from app.features.chat.models import Entity, Relationship, EntityMention
...
class EntityRepository:
    ...
    async def get_entity_relationships(
        ...
        query = select(Relationship).where(or_(*conditions))
        query = query.order_by(Relationship.strength.desc())
    ...
    async def get_related_entities(
        ...
        conditions_out = [Relationship.source_entity_id == entity_id]
        if relationship_type:
            conditions_out.append(Relationship.relationship_type == relationship_type)

        query_out = (
            select(Entity)
            .join(Relationship, Relationship.target_entity_id == Entity.entity_id)
        ...
        conditions_in = [Relationship.target_entity_id == entity_id]
        if relationship_type:
            conditions_in.append(Relationship.relationship_type == relationship_type)

        query_in = (
            select(Entity)
            .join(Relationship, Relationship.source_entity_id == Entity.entity_id)
    ...
    async def increment_relationship_mention(
        ...
        Args:
            ...
            relationship_type: 관계 타입
        """
        stmt = update(Relationship).where(
            and_(
            ...
            mention_count=Relationship.mention_count + 1,
            last_seen_at=datetime.utcnow()
        )
        await self.db.execute(stmt)
        await self.db.flush()
