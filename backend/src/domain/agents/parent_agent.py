...
# ⚙️  
# ============================================================
from ..services.orchestration import scene_tools, state_tools
from src.core.utils.tools.training_logger import log_agent

# 🎯 스테이지 핸들러 관련
from ..handlers.free_intent_handler import FreeIntentHandler
from ..handlers.mission_handler import MissionHandler
from ..handlers.router_handler import RouterStageHandler
from ..handlers.scene_handler import SceneHandler
from ..handlers.narrative_handler import OpenNarrativeHandler

from src.core.utils.logger import log

# ============================================================
# ============================================================
_state_tools_module = state_tools
...
class ParentAgent:
    ...
    def _build_context_summary(self, state: Dict[str, Any]) -> Optional[str]:
        ...
        recent_dialogues = self._collect_recent_dialogues(state, limit=4)
        
        for dialogue in recent_dialogues:
            summary_lines.append(f"{dialogue.speaker}: {dialogue.content}")
    ...
    def _collect_recent_dialogues(self, state: Dict[str, Any], limit: int = 4) -> List[Dialogue]:
        """LLM 프롬프트용으로 최근 대사 몇 줄을 정리한다."""
        recent_dialogues: List[Dialogue] = []
        history = state.get("message_history") or []
        if isinstance(history, list):
            for turn in history[-limit:]:
                if not isinstance(turn, ConversationTurn):
                    continue # Skip if not a ConversationTurn object
                if turn.user_input:
                    recent_dialogues.append(Dialogue(speaker="user", content=turn.user_input))
                recent_dialogues.extend(turn.agent_responses)
        ...
        if isinstance(output_dialogues, list):
            for dialogue in output_dialogues[-limit:]:
                if isinstance(dialogue, dict): # It might be a dict from other parts of the code
                    speaker = dialogue.get("speaker") or "unknown"
                    text = (dialogue.get("text") or dialogue.get("content") or "").strip()
                    if text:
                        recent_dialogues.append(Dialogue(speaker=speaker, content=text, emotion=dialogue.get("emotion")))
                elif isinstance(dialogue, Dialogue):
                    recent_dialogues.append(dialogue)


        return recent_dialogues[-limit:]
