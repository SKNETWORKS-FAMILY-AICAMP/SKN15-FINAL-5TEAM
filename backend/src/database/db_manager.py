...
class DatabaseManager:
    ...
    def __init__(
        ...
        Args:
            ...
            max_conn: 최대 연결 수
        """
        # 환경 변수 기반 기본값 적용 (직접 지정이 우선 적용)
        host = host or os.getenv("DB_HOST", "localhost")
        port = int(port if port is not None else os.getenv("DB_PORT", "5432"))
        ...
        min_conn = int(min_conn if min_conn is not None else os.getenv("DB_MIN_CONN", "2"))
        max_conn = int(max_conn if max_conn is not None else os.getenv("DB_MAX_CONN", "10"))
