...
# Standardized affinity rules (single source of truth)
CUTSCENE_CAP: int = 20
...
def _ensure_affinity_state(state: dict) -> Dict[str, int]:
    #문제3 : 파일 키값이 2개임 affinity or affinity_scores
    affinity = state.get("affinity") or state.get("affinity_scores") or {}
    # Keep same dict object across both keys
    state["affinity"] = affinity
    state["affinity_scores"] = affinity
    # Per-cutscene positive delta accumulator
...
def apply_rules(state: dict, bundle: Dict[str, List[str]]) -> None:
    ...
    for char_id, keys in bundle.items():
        ...
        # Positive deltas are limited by per-cutscene cap
        if total > 0:
            ...
            if apply_pos > 0:
                ...
                # No room left; still record cap_left
                cap_left_log[char_id] = 0
        else:
            # Negative deltas are not capped; apply fully then clamp
            new_val = max(0, min(1000, old + total))
            affinity[char_id] = new_val
            applied_log[char_id] = total
            # cap_left unchanged for negatives; report remaining
            used = cap_acc.get(char_id, 0)
            cap_left_log[char_id] = max(0, CUTSCENE_CAP - used)

    # keep keys in sync with same object
    state["affinity"] = affinity
    state["affinity_scores"] = affinity
